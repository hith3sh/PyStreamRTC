...
function seekClick() {
    ...
    ws_conn.send(JSON.stringify({'seek': seekTime}));
    //video.currentTime = seekTime;
}

function playClick() {
    var video = document.querySelector("video");
    ws_conn.send(JSON.stringify({'play': ''}));
}

function pauseClick() {
    var video = document.querySelector("video");
    ws_conn.send(JSON.stringify({'pause': ''}));
}
    ...
    try {
        ...
        peer_connection.setRemoteDescription(sdp).then(() => {
            ...
            if (sdp.type == "offer") {
                ...
                // peer_connection.createAnswer().then(onLocalDescription, setError);
...
function onIncomingICE(ice) {
    ...
    const candidateString = ice.candidate.replace(/\.local/g, '.127.0.0.1'); // Replace with actual GStreamer IP
